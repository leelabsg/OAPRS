...
#' @examples
#' @export
score_eval <- function(prs_res_paths,lds,target_path,output_score_path=NULL,platform="prscs",bim_path=NULL,fam_path=NULL,pheno_path=NULL,ID_col,pheno_col,cl=NULL){
  V6<-NULL
  methods=lds$methods
  pvals=lds$probs
  if(is.null(bim_path)){
  ...
  for ( j in (1:length(methods))){
    ...
    colnames(prs_res) = c("CHR","SNP","POS","A1","A2","BETA")
    ...
    scr = apply(weights,2,lassosum::pgs,bfile=target_path, extract=bim$V2 %in% prs_mkrs$V2,cluster=cl)
